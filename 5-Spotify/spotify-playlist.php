...
if (isset($_GET['id'])) {
    ...
    $result = mysqli_query($dbConn, 'SELECT * FROM playlistname WHERE id = ' . $_GET['id']);
            ...
            <div class="d-flex flex-row-reverse">
                <a href="spotify-upload.php">
                    <button class="btn btn-primary ready" type="submit" style="width: auto;">
                        +<i class="bi bi-music-note"></i> Dodaj nowy utwór
                    </button>
                </a>
            </div>
            ...
            <div class="d-flex flex-wrap">
                <table class="table">
                    ...
                    <tbody>
                        ...
                        LEFT JOIN song ON playlistdatabase.songid = song.id
                        LEFT JOIN musictype ON song.musictypeid = musictype.id
                        WHERE playlistid = $playlistid
                        ORDER BY song.title ASC;");
                        if (mysqli_num_rows($result) == 0) {
                            echo '<p>Brak utworów na tej playliście...</p>';
                        } else {
                            while ($row = mysqli_fetch_assoc($result)) {
                                echo '<tr>';
                                echo '<td class="miniature"><a href="spotify-playlist.php?id=' . $playlistid . '&songID=' . $row['ids'] . '"><img src="media/playlist/playlist.svg" alt="playlist"></a></td>';
                                echo '<td><a href="spotify-playlist.php?id=' . $playlistid . '&songID=' . $row['ids'] . '">' . $row['title'] . '</a></td>';
                                echo '<td><a href="spotify-playlist.php?id=' . $playlistid . '&songID=' . $row['ids'] . '">' . $row['musician'] . '</a></td>';
                                echo '<td><a href="spotify-playlist.php?id=' . $playlistid . '&songID=' . $row['ids'] . '">' . $row['genre'] . '</a></td>';
                                echo '</tr>';
                            }
                        }
                        mysqli_close($dbConn);
