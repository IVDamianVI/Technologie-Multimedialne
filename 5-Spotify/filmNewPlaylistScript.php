...
{
    $stmt = mysqli_prepare($dbConn, 'SELECT name FROM filmplaylistname WHERE creator = ? AND name = ?');
...
$stmt = mysqli_prepare($dbConn, "INSERT INTO filmplaylistname (creator, name, public) VALUES (?, ?, ?)");
...
$result = mysqli_query($dbConn, "SELECT * FROM filmplaylistname WHERE name = '$name' AND creator = '$user';");
$row = mysqli_fetch_assoc($result);
$newPlaylistID = $row['id'];

mysqli_close($dbConn);
header('Location: film-playlist.php?id=' . $newPlaylistID);
exit();
