            ...
            <div class="d-flex flex-wrap">
                <table class="table">
                    ...
                    <tbody>
                        ...
                        require('access.php');
                        $dbConn = mysqli_connect($dbHost, $dbUsername, $dbPassword, $dbDatabase);
                        $result = mysqli_query($dbConn, 'SELECT film.id AS idf, film.title AS title, film.director AS musician, filmtype.name AS genre
                        FROM film
                        LEFT JOIN filmtype ON film.filmtypeid = filmtype.id
                        ORDER BY film.title ASC');
                        if (mysqli_num_rows($result) == 0) {
                            echo '<p>Brak filmów w bibliotece...</p>';
                        } else {
                            while ($row = mysqli_fetch_assoc($result)) {
                                echo '<tr>';
                                echo '<td class="miniature"><a href="film-playlist.php?id='.$row['idf'].'"><img src="media/playlist/filmplaylist.svg" alt="playlist"></a></td>';
                                echo '<td><a href="film.php?id='.$row['idf'].'">' . $row['title'] . '</a></td>';
                                echo '<td><a href="film.php?id='.$row['idf'].'">' . $row['musician'] . '</a></td>';
                                echo '<td><a href="film.php?id='.$row['idf'].'">' . $row['genre'] . '</a></td>';
                                echo '</tr>';
                            }
                        }
                        mysqli_close($dbConn);
