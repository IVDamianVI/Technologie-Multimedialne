...
if (isset($_GET['id'])) {
    ...
    $result = mysqli_query($dbConn, 'SELECT * FROM filmplaylistname WHERE id = ' . $_GET['id']);
    $row = mysqli_fetch_assoc($result);
    $playlistid = $row['id'];
    $_SESSION['currentPlaylist'] = $row['name'];
            ...
            <div class="d-flex flex-row-reverse">
                <a href="film-upload.php">
                    <button class="btn btn-primary ready" type="submit" style="width: auto;">
                        +<i class="bi bi-film"></i> Dodaj nowy film
                    </button>
                </a>
            </div>
            ...
            <div class="d-flex flex-wrap">
                <table class="table">
                    ...
                    <tbody>
                        ...
                        require('access.php');
                        $dbConn = mysqli_connect($dbHost, $dbUsername, $dbPassword, $dbDatabase);
                        $result = mysqli_query($dbConn, "SELECT film.id AS ids, film.title AS title, film.director AS musician, filmtype.name AS genre
                        FROM filmplaylistdatabase
                        LEFT JOIN film ON filmplaylistdatabase.filmid = film.id
                        LEFT JOIN filmtype ON film.filmtypeid = filmtype.id
                        WHERE playlistid = $playlistid
                        ORDER BY film.title ASC;");
                        if (mysqli_num_rows($result) == 0) {
                            echo '<p>Brak filmów na tej playliście...</p>';
                        } else {
                            while ($row = mysqli_fetch_assoc($result)) {
                                echo '<tr>';
                                echo '<td class="miniature"><a href="film.php?id=' . $row['ids'] . '"><img src="media/playlist/filmplaylist.svg" alt="playlist"></a></td>';
                                echo '<td><a href="film.php?id=' . $row['ids'] . '">' . $row['title'] . '</a></td>';
                                echo '<td><a href="film.php?id=' . $row['ids'] . '">' . $row['musician'] . '</a></td>';
                                echo '<td><a href="film.php?id=' . $row['ids'] . '">' . $row['genre'] . '</a></td>';
                                echo '</tr>';
                            }
                        }
                        mysqli_close($dbConn);
