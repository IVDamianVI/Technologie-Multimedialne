    ...
    <main>
        ...
        <div class="content">
            ...
            <div class="d-flex flex-wrap">
                ...
                require('access.php');
                $dbConn = mysqli_connect($dbHost, $dbUsername, $dbPassword, $dbDatabase);
                $result = mysqli_query($dbConn, 'SELECT * FROM song ORDER BY id DESC LIMIT 5');
                if (mysqli_num_rows($result) == 0) {
                    echo '<p>Brak nowych utworów w ostatnim czasie...</p>';
                } else {
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo '<a class="song-link" href="spotify.php?songID=' . $row['id'] . '">';
                        ...
                        echo '<p class="playlist-item-info">' . $row['musician'] . '</p>';
                        echo '</div>';
                        echo '</div></a>';
            ...
            <div class="d-flex flex-wrap">
                ...
                require('access.php');
                $dbConn = mysqli_connect($dbHost, $dbUsername, $dbPassword, $dbDatabase);
                $result = mysqli_query($dbConn, 'SELECT * FROM playlistname ORDER BY id DESC LIMIT 5');
                if (mysqli_num_rows($result) == 0) {
                    echo '<p>Brak nowych playlist w ostatnim czasie...</p>';
                } else {
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo '<a class="song-link" href="spotify-playlist.php?id=' . $row['id'] . '">';
                        echo '<div class="playlist-item">';
                        echo '<div class="playlist-item-img">';
                        echo '<img src="media/playlist/playlist.svg" alt="playlist">';
                        echo '</div>';
