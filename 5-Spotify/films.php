    ...
    <main>
        ...
        <div class="content">
            ...
            <div class="d-flex flex-wrap">
                ...
                require('access.php');
                $dbConn = mysqli_connect($dbHost, $dbUsername, $dbPassword, $dbDatabase);
                $result = mysqli_query($dbConn, 'SELECT * FROM film ORDER BY id DESC LIMIT 5');
                if (mysqli_num_rows($result) == 0) {
                    echo '<p>Brak nowych filmów w ostatnim czasie...</p>';
                } else {
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo '<a class="song-link" href="film.php?id=' . $row['id'] . '">';
                        echo '<div class="playlist-item">';
                        echo '<div class="playlist-item-img">';
                        echo '<img src="media/playlist/filmplaylist.svg" alt="playlist">';
                        echo '</div>';
                        echo '<div class="playlist-item-text">';
                        echo '<p class="playlist-item-title">' . $row['title'] . '</p>';
                        echo '<p class="playlist-item-info">' . $row['director'] . '</p>';
                        echo '</div>';
                        echo '</div></a>';
            ...
            <div class="d-flex flex-wrap">
                ...
                require('access.php');
                $dbConn = mysqli_connect($dbHost, $dbUsername, $dbPassword, $dbDatabase);
                $result = mysqli_query($dbConn, 'SELECT * FROM filmplaylistname ORDER BY id DESC LIMIT 5');
                if (mysqli_num_rows($result) == 0) {
                    echo '<p>Brak nowych playlist w ostatnim czasie...</p>';
                } else {
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo '<a class="song-link" href="film-playlist.php?id=' . $row['id'] . '">';
                        echo '<div class="playlist-item">';
                        echo '<div class="playlist-item-img">';
                        echo '<img src="media/playlist/filmplaylist.svg" alt="playlist">';
                        echo '</div>';
                        echo '<div class="playlist-item-text">';
                        echo '<p class="playlist-item-title">' . $row['name'] . '</p>';
                        echo '<p class="playlist-item-info">Playlista &centerdot;
                            ' . $row['creator'] . '
                        </p>';
                        echo '</div>';
                        echo '</div>';
                ...
                <!-- <div class="playlist-item">
                    ...
                    <div class="playlist-item-text">
                        <p class="playlist-item-title">Tytuł</p>
                        <p class="playlist-item-info">Playlista &centerdot; Autor</p>
                    </div>
                </div>
            ...
            <div class="d-flex flex-wrap">
                <div class="playlist-item">
                    ...
                    <div class="playlist-item-text">
                        <p class="playlist-item-title">Placeholder</p>
                        <p class="playlist-item-info">Placeholder</p>
                    </div>
                </div>
                <div style="width: 25px;"></div>
                <div class="playlist-item">
